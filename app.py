...
def perform_segmentation(segmentation_model_path, image_path):
    ...
    im_array = segmentation_results[0].plot(
        line_width=1, conf=0.5, boxes=False
    )  # BGR numpy array of predictions
...
def preprocess_and_predict(image_path, detection_model_path, segmentation_model_path):
    try:
        ...
        detection_results = detection_model.predict(
            source=preprocessed_image_path, conf=0.55
        )
...
def load_image_from_url(url):
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(
                "Failed to fetch the image. Status code: " + str(response.status_code)
            )
            return None
...
def assemble_model_file(parts_folder, original_filename):
    ...
    with open(assembled_file_path, "wb") as assembled_file:
        ...
        while True:
            part_filename = (
                f"{os.path.join(parts_folder, original_filename)}.part{part_number}"
            )
...
def main():
    ...
    st.markdown(
        "This app uses a YOLOv8 ensemble model to perform object detection and segmentation on dental xray images."
    )
    ...
    **Note**: The model may take a few seconds to process the image. Please be patient.
    ...
    cols = st.columns(len(urls))  # Create a column for each image
    ...
    if image_path is not None and st.button("Run Model"):
        ...
        if image_path:
            ...
            processed_image = preprocess_and_predict(
                image_path, detection_model_path, segmentation_model_path
            )
